...
class CompanyController extends Controllermaster
{
   ...
   { $this->model = new Company;
      ...
      $this->statusta = [
         ...
         array(
            "comboValue" => 5,
            "comboLabel" => "Pergeseran APBD",
         ),
         array(
            "comboValue" => 6,
            "comboLabel" => "APBD Perubahan I"
         ),

         array(
            "comboValue" => 7,
            "comboLabel" => "Pergeseran Setelah Perubahan APBD",
         ),
         array(
            "comboValue" => 8,
            "comboLabel" => "APBD Perubahan II"
         ),
      ];
      ...
      $this->form = array(
         ...
         array(
            ...
            'field' => 'compKotaKlien',
            'type' => 'text',
            'placeholder' => 'Masukan Kota Klien',
            'keterangan' => '* Wajib Diisi'
         ),
         array(
            'label' => 'NAMA LOGO KLIEN',
            'field' => 'compLogoKlien',
            'type' => 'text',
            'placeholder' => 'Masukan Logo Klien',
            'keterangan' => '* Wajib Diisi'
         ),
         // array(
         //     'label' => 'STATUS ANGGARAN',
         //     'field' => 'compStatusAnggaran',
         //     'type' => 'combo',
         //     'combodata' => $this->statusta,
         //     'default' => 'Pilih Status',
         //     'keterangan' => '* Wajib Diisi'
         // ),
   ...
   public function index()
   {
      if (trim(Session::get('email')) == '' or $this->checkRouteAuth() == 2) {
         ...
         } else {       $search = !empty($_GET['search']) ? $_GET['search'] : '';
            if ($search == '') {
               $listdata = $this->model->select(
                  '*',
                  DB::raw('if(compStatus = 1,"Administrator Utama",
   ...
   public function update(Request $request, $id)
   {
      ...
      if ($request->hasFile('compLogo') == true) {    // $img =  $request->file('compLogo');
   ...
   public function setCompany(Request $request)
   {
      ...
      if ($request->compKategori == 1) {    $menu = [
